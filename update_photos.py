...
personas = Person.objects.filter(biography__isnull=True) | Person.objects.filter(biography='')
total = personas.count()
print(f"Actores sin biografía: {total}")

for i, person in enumerate(personas, 1):
    try:
        # Busca el actor por nombre
        r = requests.get(
        ...
        # Trae el detalle completo del actor
        r2 = requests.get(
